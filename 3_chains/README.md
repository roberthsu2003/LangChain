...
[官方說明書-chain](https://python.langchain.com/api_reference/langchain/chains.html)
...
| 順序 | 類型 | 難度 | 核心概念 | 學習重點 |
|------|------|------|----------|----------|
| **1** | **基礎鏈** | ⭐ | LCEL 語法 | 管道符號 `\|` 的基本用法 |
| **2** | **內部運作** | ⭐⭐⭐⭐ | 底層機制 | RunnableSequence 手動組合 |
| **3** | **擴展鏈** | ⭐⭐ | 自定義處理 | RunnableLambda 的使用 |
| **4** | **並行鏈** | ⭐⭐⭐ | 效率優化 | RunnableParallel 並行執行 |
| **5** | **分支鏈** | ⭐⭐⭐ | 智能路由 | RunnableBranch 條件判斷 |
| **6** | **Lambda 模型整合** | ⭐⭐⭐⭐⭐ | 進階技巧 | 動態模型調用 |
...
## 2️⃣ 鏈的內部運作 (Chains Under the Hood) - 深入理解
...
### 📁 **範例檔案**
- [擴展鏈 - Ollama 版本](3_chains_extended_ollama.ipynb) ⭐ **推薦初學者**
...
### 📁 **範例檔案**
- [並行鏈 - Ollama 版本](4_chains_parallel_ollama.ipynb) ⭐ **推薦初學者**
- [並行鏈 - Gemini 版本](4_chains_parallel_gemini.ipynb)

---

## 5️⃣ 分支鏈 (Branching Chains) - 智能路由
...
### 📁 **範例檔案**
- [分支鏈 - Ollama 版本](5_chains_branching_ollama.ipynb) ⭐ **推薦初學者**
- [分支鏈 - Gemini 版本](5_chains_branching_gemini.ipynb)


## 🚀 學習路徑建議

本教材提供**雙軌制學習路徑**，您可以根據自己的學習目標選擇適合的路徑：

### 🚀 **快速實戰路徑**（重實用，推薦初學者）
適合想要快速上手、注重實際應用的學習者

1. **基礎鏈** (1️⃣) → 學習基本的 LCEL 語法
2. **擴展鏈** (3️⃣) → 添加自定義處理邏輯
3. **並行鏈** (4️⃣) → 提升處理效率
4. **分支鏈** (5️⃣) → 實現智能路由
5. **內部運作** (2️⃣) → 深入理解底層機制
6. **Lambda 模型整合** (6️⃣) → 掌握最高階技巧

**優點**：難度平滑上升（⭐ → ⭐⭐ → ⭐⭐⭐ → ⭐⭐⭐ → ⭐⭐⭐⭐ → ⭐⭐⭐⭐⭐），每步都能立即應用到實際專案中

---

### 🔬 **深度理解路徑**（重原理，推薦進階開發者）
適合想要深入理解 LangChain 內部機制、追求技術深度的學習者

1. **基礎鏈** (1️⃣) → 學習基本的 LCEL 語法
2. **內部運作** (2️⃣) → 深入理解底層機制
3. **擴展鏈** (3️⃣) → 自定義功能（包含進階 Lambda 模型整合）
4. **並行鏈** (4️⃣) → 優化效能
5. **分支鏈** (5️⃣) → 實現複雜邏輯
6. **Lambda 模型整合** (6️⃣) → 掌握動態模型調用

**優點**：先理解「為什麼」再學「怎麼做」，有助於寫出更優化、更易維護的程式碼
...
| 情境 | 推薦路徑 | 原因 |
|------|----------|------|
| 🎯 我想快速開發 AI 應用 | 快速實戰路徑 | 循序漸進，立即可用 |
| 📚 我想深入理解 LangChain | 深度理解路徑 | 知其然知其所以然 |
| 🔧 我要為團隊建立最佳實踐 | 深度理解路徑 | 需要優化和除錯能力 |
| 🚀 我是 AI 開發新手 | 快速實戰路徑 | 避免過早陷入複雜細節 |
| 🎓 我有程式開發經驗 | 深度理解路徑 | 能更快掌握核心原理 |
...
### Q: Lambda 模型整合什麼時候使用？
A: 當你需要在處理過程中動態調用模型，或實現複雜的多步驟推理時使用。
...
| **Lambda 模型整合** | 動態模型調用 | Lambda + Model | 多步驟推理、動態處理 | 智能文章生成 | ⭐⭐⭐⭐⭐ |
...
### 🛠️ **實戰專案建議**
1. **智能客服系統** - 結合分支鏈和記憶功能（參考案例 1）
2. **內容創作助手** - 使用並行鏈生成多版本內容（參考案例 2）
